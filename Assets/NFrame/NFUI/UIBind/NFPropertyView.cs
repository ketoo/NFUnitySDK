			...
			case ViewType.SKILL_CNFID_ICON:
				{
                    ...
                    {
                        string strCnfID = data.StringVal();
                        string strIconName = mElementModule.QueryPropertyString (strCnfID, NFrame.Skill.Icon);
						string strIconFileName = mElementModule.QueryPropertyString (strCnfID, NFrame.Skill.SpriteFile);
						Sprite xSprite = NFTexturePacker.Instance.GetSprit (strIconFileName, strIconName);
						if (xSprite != null)
						{
							xImage.overrideSprite = xSprite;
